...
- **OBSERVED_PLAY_MEMORY_ENABLED flag control fix (session 57)** — `docker-compose.override.yml` previously hard-coded `OBSERVED_PLAY_MEMORY_ENABLED: "true"` with no easy way to turn off without editing committed files. Now uses `"${OBSERVED_PLAY_MEMORY_ENABLED:-false}"` on both `backend` and `celery-worker` services so the flag defaults to off and can be controlled via `.env` (gitignored). Added commented-out entry to `.env.example` explaining the pattern. To enable: `echo "OBSERVED_PLAY_MEMORY_ENABLED=true" >> .env && docker compose up -d --no-deps backend celery-worker`. To disable: remove the line and re-up. Both containers confirmed reporting `False` with flag absent from `.env`.

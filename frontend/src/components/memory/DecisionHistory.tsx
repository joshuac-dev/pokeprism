...
export default function DecisionHistory({ cardId }: Props) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5">
      <h3 className="text-white font-semibold mb-4">Decision History</h3>
            ...
            <tbody className="divide-y divide-slate-800">
              {rows.map(d => (
                <tr key={d.id} className="hover:bg-slate-800/50">
                  <td className="py-2 pr-4 text-slate-400">{d.turn_number}</td>
      ...
      {hasMore && !loading && (
        <button
          ...
          className="mt-4 px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm"
        >

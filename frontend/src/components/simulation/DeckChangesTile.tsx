...
function WrBadge({ value }: { value: number | null }) {
  if (value == null) return <span className="text-app-text-muted">—</span>;
...
function DeltaBadge({ before, after }: { before: number | null; after: number | null }) {
  ...
  const cls = delta > 0 ? 'text-ctp-green' : delta < 0 ? 'text-ctp-red' : 'text-app-text-subtle';
  return (
  ...
  return (
    <div className="bg-app-surface border border-app-border rounded-lg flex flex-col">
      ...
      <div className="px-4 py-3 border-b border-app-border flex items-center justify-between">
        <h3 className="text-sm font-semibold text-app-text">Deck Changes</h3>
        <span className="text-xs text-app-text-muted">{mutations.length} swap{mutations.length !== 1 ? 's' : ''}</span>
      </div>

      {/* Body */}
      <div className="flex-1 overflow-y-auto max-h-64">
        {mutations.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-app-text-muted text-sm gap-1">
          ...
          <ul className="divide-y divide-slate-200/50 dark:divide-slate-700/50">
            {mutations.map((m, i) => (
              <li key={i} className="px-4 py-2.5 flex items-start gap-3 text-xs">
                {/* Round badge */}
                <span className="mt-0.5 shrink-0 bg-slate-100 dark:bg-slate-700 text-app-text-muted rounded px-1.5 py-0.5 font-mono text-[11px]">
                  R{m.round}
                </span>
                ...
                <div className="flex-1 min-w-0">
                  {m.card_out && (
                    <p className="text-ctp-red truncate">
                      <span className="opacity-60">−</span> {m.card_out}
                    </p>
                  )}
                  {m.card_in && (
                    <p className="text-ctp-green truncate">
                      <span className="opacity-60">+</span> {m.card_in}
                    </p>
                  )}
                  {!m.card_in && !m.card_out && (
                    <p className="text-app-text-muted italic">deck mutation</p>
                  )}
                </div>

...
from sqlalchemy import and_, asc, case, delete, desc, distinct, func, or_, select
...
from app.observed_play.schemas import (
    ...
    ObservedPlayCoachContextQuery,
)
from app.observed_play.readiness_service import (
    compute_corpus_readiness as _compute_corpus_readiness,
    build_coach_evidence_filter as _build_coach_evidence_filter,
)
...
# ── Phase 5.2: Corpus Readiness (delegated to readiness_service) ──────────────
...
@router.get("/coach-context-preview")
async def get_coach_context_preview(
    card_name: Optional[str] = Query(None),
    action_name: Optional[str] = Query(None),
    memory_type: Optional[str] = Query(None),
    player_alias: Optional[str] = Query(None),
    ...
    """Read-only preview of the observed-play evidence block that would be
    injected into a Coach prompt when OBSERVED_PLAY_MEMORY_ENABLED=true.

    When the flag is false, returns enabled=false and an empty prompt_block.
    When the flag is true, returns the formatted evidence block, evidence IDs,
    readiness verdict, and any warnings from the corpus readiness gate.

    This endpoint never mutates the database.  The prompt_block is advisory
    only and must not override card rules, game state, simulator results, or
    card database facts.
    """

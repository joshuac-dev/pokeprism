#!/usr/bin/env python3
"""Insert Batch 9 card definitions into the PostgreSQL cards table.

Cards covered:
  - JTG (sv09): 042–139

Pre-fetched card data is loaded from batch9_cards.json in the project root.
Fixtures are saved to tests/fixtures/cards/ for future use.

Already in DB (excluded): sv09-056, sv09-098, sv09-116, sv09-120
Also excluded (prior batch): sv09-038, sv09-039

Usage:
    cd backend && python3 -m scripts.add_batch9_cards
"""
...
BATCH9_JSON = Path(__file__).parent.parent.parent / "batch9_cards.json"
...
_ALREADY_IN_DB = {"sv09-056", "sv09-098", "sv09-116", "sv09-120",
                  "sv09-038", "sv09-039"}

# Build full list of card IDs to process: sv09-042..sv09-139, minus already-in-DB
ALL_IDS = [
    f"sv09-{n:03d}" for n in range(42, 140)
    if f"sv09-{n:03d}" not in _ALREADY_IN_DB
]
...
async def main() -> None:
    ...
    # Load pre-fetched data from batch9_cards.json if available
    prefetched: dict[str, dict] = {}
    if BATCH9_JSON.exists():
        try:
            prefetched = json.loads(BATCH9_JSON.read_text(encoding="utf-8"))
            logger.info("Loaded %d pre-fetched cards from batch9_cards.json", len(prefetched))
        except Exception as exc:
            logger.warning("Could not load batch9_cards.json: %s", exc)
    ...
    for card_id in ALL_IDS:
        ...
        # 2. Fall back to pre-fetched batch9_cards.json data
